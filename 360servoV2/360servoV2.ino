...
//translate from 0 to servoAnalogRange to 0 to 360
int servoAnalogRange = 451;
int NEUTRAL_POSITION = 20;//position where it is unfolded (can be changed)
int PULL_OFFSET = 67.78;//how much it actuates in each direction during the folded state
float FOLDED_POSITION = NEUTRAL_POSITION + 0.5*servoAnalogRange; //position where it is folded
...
void loop()
{
  ...
  if (stage == LEFT)
  {
    ...
    moveToPosition(LEFT_POSITION, "clockwise", 50); //move to left pos
    myservo.write(stopSpeed);
  ...
  if (stage == RIGHT)
  {
    ...
    {
      moveToPosition(FOLDED_POSITION, "clockwise", 50);
      myservo.write(stopSpeed);
      Serial.println("movedToCenter");
    }
    else if (actuationCompleted == false) //tell it to move right or stop moving
  ...
  if (stage == DONE)
  {
    moveToPosition(NEUTRAL_POSITION, "clockwise", 50); //move clockwise to neutral positon to return to unfolded state
    delay(100);
    myservo.write(stopSpeed);
    moves = 0;
...
{ //this is the main function to move to a designated position as called in the state machine
  ...
  {
    while (readAnalog() > ((positionValue / 360) * servoAnalogRange) + errorRange1)
    {
      // location = readAnalog();
